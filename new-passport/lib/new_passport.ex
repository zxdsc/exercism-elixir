defmodule NewPassport do
  def get_new_passport(now, birthday, form) do
    with {:ok, timestamp} <- enter_building(now),
         ...
         number_of_counters <- manual.(birthday),
         {:ok, checksum} <- stamp_form(timestamp, number_of_counters, form),
         passport_number <- get_new_passport_number(timestamp, number_of_counters, checksum) do
      {:ok, passport_number}
    else
